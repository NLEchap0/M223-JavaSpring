...
public class CustomerControllerTests {
    ...
    public void testLoadInsertPage() throws Exception {
        mockMvc.perform(get("/"))
                .andExpect(status().isOk())  // Verifica che lo stato sia OK (200)
                .andExpect(view().name("home"))  // Verifica che la vista sia "home"
    ...
    public void testSaveCustomer_Success() throws Exception {
        ...
        mockMvc.perform(post("/addCustomer")
                        ...
                        .param("age", "70")
                        .param("city", "London")
                        .param("ccNumber", "7498184758291749")
                        .param("ccExpiration", "5/29")
                        .param("ccCVV", "943"))
                .andExpect(status().is3xxRedirection())  // Verifica che ci sia una redirezione come implementato nel metodo => return "redirect:/"
                .andExpect(redirectedUrl("/"));  // Verifica la redirezione a /
    ...
    @Test
    public void testLoadCustomersById() throws Exception {
        mockMvc.perform(get("/loadCustomers?id=1"))
                .andExpect(status().isOk())  // Verifica che lo stato sia OK (200)
                .andExpect(view().name("customerList"))  // Verifica che la vista sia "customerList"
                .andExpect(model().attributeExists("customers"))  // Verifica che il modello contenga "customers"
                .andExpect(model().attribute("customers", customerRepository.findById(1L).orElse(null)));  // Verifica che "customer" contenga il risultato della query
    }
